...
describe('containerMatcher / workloadMatcher', () => {
  it('encodes service_name as container / deployment, and never as a namespace matcher', () => {
    expect(containerMatcher(withNamespace)).toBe('{cluster="k8s-rome-sec-test",container="rome-sec-admin"}');
    expect(containerMatcher(withoutNamespace)).toBe('{cluster="k8s-rome-sec-test",container="rome-sec-admin"}');
    expect(containerMatcher(withNamespace)).not.toContain('namespace=');
  ...
  it('keys kube_deployment_* by deployment, which is the stage-0 encoding of service_name', () => {
    ...
    expect(workloadMatcher(withNamespace)).toBe('{cluster="k8s-rome-sec-test",deployment="rome-sec-admin"}');
  });
});
...
describe('spanmetricsMatcher / clusterMatcher', () => {
  it('matches spanmetrics on service_name alone, even when a namespace is on the page', () => {
    expect(spanmetricsMatcher(withNamespace)).toBe('{service_name="rome-sec-admin"}');
  });

  it('filters cluster-only families by cluster plus extras', () => {
...
describe('podSetFilter', () => {
  it('intersects on pod through a container-labelled metric instead of a pod name regex', () => {
    expect(podSetFilter(withoutNamespace)).toBe(
    ...
    expect(podSetFilter(withNamespace)).not.toContain('namespace=');
  });
});
...
describe('scopeMatcher', () => {
  it('is cluster only — namespace is display, not a matcher', () => {
    expect(scopeMatcher(withNamespace)).toBe('{cluster="k8s-rome-sec-test"}');
