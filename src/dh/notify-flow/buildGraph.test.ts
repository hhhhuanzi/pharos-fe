...
const MESSAGES: Record<string, string> = {
  ...
  'notification_configuration.filters.no_extra': '时段/标签/属性不限',
...
describe('formatFilterLines / formatFiltersSummary', () => {
  it('默认筛选（全级别且无时段/标签/属性）显示友好摘要，而不是字段名或 [1,2,3]', () => {
    const lines = formatFilterLines({ severities: [1, 2, 3] }, t);
    expect(lines).toEqual(['全部级别 · 时段/标签/属性不限']);
    expect(formatFiltersSummary({ severities: [1, 2, 3] }, t)).toBe('全部级别 · 时段/标签/属性不限');
    for (const key of TECH_FILTER_KEYS) {
      expect(lines.join(' ')).not.toContain(key);
    }
    expect(lines.join(' ')).not.toContain('[1');
    expect(lines.join(' ')).not.toContain('1,2,3');
  });

  it('未勾选级别时提示不会匹配', () => {
    expect(formatFilterLines({ severities: [] }, t)).toEqual(['未勾选级别，不会匹配任何事件 · 时段/标签/属性不限']);
    expect(formatFilterLines({}, t)).toEqual(['未勾选级别，不会匹配任何事件 · 时段/标签/属性不限']);
  });
    ...
    const lines = formatFilterLines(
      {
        ...
        label_keys: [{ key: 'app', op: '==', value: 'api' }, { key: 'namespace', op: 'not in', value: 'prod staging extra-very-long-name' }],
...
describe('buildNotifyFlowGraph', () => {
  ...
  it('N 条配置 → 1 source + N 条 filter→channel→template 链（并行扇出，无 XOR）', () => {
    ...
    const texts = graph.nodes.flatMap((node) => {
      ...
      if (node.kind === 'filter') return node.lines;
  ...
  it('筛选/媒介/模板分框；媒介框不含模板名', () => {
    ...
    expect(filter?.lines).toEqual(['全部级别 · 时段/标签/属性不限']);
  ...
  it('未选媒介/模板用占位文案；坐标从左到右且可重复调用（幂等）', () => {
    ...
    expect(filters[0].position.x < channels[0].position.x).toBe(true);
    expect(channels[0].position.x < templates[0].position.x).toBe(true);
    expect(filters[0].position.y).toBe(LAYOUT.paddingY);
    expect(channels[0].position.y).toBe(filters[0].position.y);
    expect(templates[0].position.y).toBe(filters[0].position.y);
    expect(filters[1].position.y).toBe(LAYOUT.paddingY + LAYOUT.rowHeight);
